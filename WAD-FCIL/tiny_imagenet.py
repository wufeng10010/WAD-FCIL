...
class Tiny_Imagenet:
    ...
    def get_data(self):
        ...
        class_path = [d for d in os.listdir(train_path) if os.path.isdir(os.path.join(train_path, d))]
        class_path = sorted(class_path)

        self.tgt_idx_to_class = {i: class_path[i] for i in range(len(class_path))}
        self.class_to_tgt_idx = {class_path[i]: i for i in range(len(class_path))}
